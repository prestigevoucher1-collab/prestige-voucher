      ...
      <div className="bg-white text-[#1e293b] min-h-screen font-body text-xs md:text-base selection:bg-[#1565d8] selection:text-white">
         ...
         <main className={`relative z-10 overflow-hidden ${isMenuOpen ? 'hidden' : 'block'}`}>
            ...
            <header className="pt-20 md:pt-28 pb-12 md:pb-20 px-4 md:px-6 lg:px-16 bg-gradient-to-br from-[#f0f7ff] to-white relative overflow-hidden">
               ...
               <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-center relative z-10">
                  ...
                  <div className="relative" id="purchase">
                     {/* Quick Tag - Decreased movement intensity */}
                     <div className="absolute -top-6 -right-6 md:-right-10 bg-white p-4 rounded-2xl shadow-xl border border-slate-50 z-20 hidden md:flex items-center gap-3 animate-subtle-bounce">
                     ...
                     <div className="bg-white rounded-[2rem] md:rounded-[3rem] shadow-3xl p-8 md:p-12 border-4 border-[#1565d8]/10 relative overflow-hidden">
                        ...
                        {submitted ? (
                           ...
                           <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
                              <input name="fullName" value={formData.fullName} onChange={handleInputChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 outline-none focus:border-[#1565d8] focus:ring-1 focus:ring-[#1565d8]/20 font-bold text-sm md:text-base transition-all placeholder:text-slate-300" placeholder="Full Name *" required />
                              <input name="phone" value={formData.phone} onChange={handleInputChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 outline-none focus:border-[#1565d8] focus:ring-1 focus:ring-[#1565d8]/20 font-bold text-sm md:text-base transition-all placeholder:text-slate-300" placeholder="Mobile Number *" maxLength={10} required />
                              <input name="email" value={formData.email} onChange={handleInputChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 outline-none focus:border-[#1565d8] focus:ring-1 focus:ring-[#1565d8]/20 font-bold text-sm md:text-base transition-all placeholder:text-slate-300" placeholder="Email Address *" type="email" required />

                              <div className="relative">
                                 <select name="state" value={formData.state} onChange={handleInputChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 outline-none focus:border-[#1565d8] focus:ring-1 focus:ring-[#1565d8]/20 font-bold text-sm md:text-base appearance-none cursor-pointer transition-all" required>
                              ...
                              <div className="relative">
                                 <select name="quantity" value={formData.quantity} onChange={handleInputChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-6 py-4 outline-none focus:border-[#1565d8] focus:ring-1 focus:ring-[#1565d8]/20 font-bold text-sm md:text-base appearance-none cursor-pointer transition-all" required>
                              ...
                              <button type="submit" className="w-full bg-[#1565d8] text-white py-4 md:py-5 rounded-xl font-black text-sm md:text-lg uppercase tracking-[0.2em] hover:bg-[#091e42] transition-all shadow-xl mt-4 flex items-center justify-center gap-2">
            ...
            <section className="py-16 md:py-24 px-4 md:px-6 lg:px-16 bg-slate-50 overflow-hidden">
               ...
               {/* Comparison Section - Compacted per request */}
               <section className="py-16 md:py-24 px-4 md:px-6 lg:px-16 bg-white relative">
                  <div className="max-w-[1200px] mx-auto">
                     <div className="text-center mb-10 md:mb-16">
                        <h2 className="text-2xl md:text-4xl font-black text-[#091e42] mb-2 uppercase tracking-tighter">Why Fryment Wins?</h2>
                        <p className="text-[#1565d8] font-bold tracking-[0.3em] uppercase text-[9px] md:text-xs">Direct Comparison</p>
                     </div>
                     <div className="bg-white rounded-2xl md:rounded-[3rem] shadow-2xl border-2 border-slate-50 max-w-5xl mx-auto overflow-hidden">
                        <div className="overflow-x-auto">
                           <table className="w-full text-left table-auto">
                              <thead><tr className="bg-[#1565d8] text-white"><th className="px-4 md:px-10 py-4 md:py-5 font-black uppercase text-[10px] md:text-xs tracking-widest whitespace-nowrap">Features</th><th className="px-4 md:px-10 py-4 md:py-5 font-black uppercase text-[10px] md:text-xs tracking-widest text-[#fbbc04] whitespace-nowrap">Fryment</th><th className="px-4 md:px-10 py-4 md:py-5 font-black uppercase text-[10px] md:text-xs tracking-widest opacity-40 whitespace-nowrap">Others</th></tr></thead>
                              <tbody>{comparisonPoints.map((row, i) => (<tr key={i} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors"><td className="px-4 md:px-10 py-4 md:py-5 font-black text-[#091e42] text-[10px] md:text-base">{row.feature}</td><td className="px-4 md:px-10 py-4 md:py-5 font-bold text-[#1565d8] text-[10px] md:text-base flex items-center gap-2"><span className="material-icons text-green-500 text-xs md:text-base">verified</span> {row.prestige}</td><td className="px-4 md:px-10 py-4 md:py-5 font-medium text-slate-400 text-[10px] md:text-base opacity-60">{row.others}</td></tr>))}</tbody>
                           </table>
                        </div>
                     </div>
                  </div>
            ...
            <section className="py-16 md:py-24 px-4 md:px-6 lg:px-16 bg-white" id="blogs">
               <div className="max-w-[1920px] mx-auto">
                  <div className="mb-10 md:mb-16 text-center">
                     <h2 className="text-2xl md:text-4xl font-black text-[#091e42]">PTE Resources</h2>
                     <Link href="/pte/blog" className="text-sm font-bold text-[#1565d8] hover:underline mt-2 inline-block">View All Articles &rarr;</Link>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
                     {liveBlogs.map((blog, i) => (
                        <Link href={`/pte/blog/${blog.slug}`} key={i} className="bg-white rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-xl border border-slate-100 transition-all hover:-translate-y-3 block">
                           <div className="aspect-video overflow-hidden">
                              <img src={blog.feature_img_url || 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?q=80&w=400&auto=format&fit=crop'} className="w-full h-full object-cover" />
                           </div>
                           <div className="p-8 md:p-12 space-y-4">
                              <h4 className="text-xl md:text-2xl font-black text-[#091e42] leading-tight line-clamp-2">{blog.title}</h4>
                              <p className="text-sm md:text-base text-slate-500 font-medium leading-relaxed line-clamp-3">{blog.excerpt || 'Read the full guide on Fryment.'}</p>
                           </div>
                        </Link>
                     ))}
                  </div>
         ...
         {!isMenuOpen && (
            <footer className="bg-[#091e42] pt-20 md:pt-32 pb-12 md:pb-16 px-4 md:px-6 lg:px-16 text-white text-center md:text-left relative z-10">
               ...
               <div className="max-w-[1920px] mx-auto mt-20 pt-10 border-t border-white/5 text-center">
                  <p className="text-white/20 text-[10px] md:text-xs font-black uppercase tracking-[0.3em]">© 2026 Fryment. Authorized Pearson Partner.</p>
               </div>
            </footer>
         )}
